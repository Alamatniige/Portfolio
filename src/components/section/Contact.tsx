...
export default function Contact() {
  ...
  const sendEmail = async (templateParams: {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "EmailJS is not configured. Please set up your .env file with EmailJS credentials.",
        );
      }
      ...
      if (errorObj?.text?.includes("insufficient authentication scopes")) {
        throw new Error(
          "Gmail authentication issue. Please reconnect your Gmail service in EmailJS dashboard.",
        );
      }

      if (
        errorObj?.text?.includes("Invalid service ID") ||
        errorObj?.text?.includes("Invalid template ID")
      ) {
        throw new Error(
          "Invalid EmailJS configuration. Please check your Service ID and Template ID.",
        );
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMsg =
        errorObj?.message || errorObj?.text || "Unknown error occurred";
      setStatusMessage(
        errorMsg.includes("authentication") || errorMsg.includes("scopes")
          ? "Email service authentication error. Please contact me directly at " +
              RECIPIENT_EMAIL
  ...
  return (
    <section
      ref={sectionRef}
      id="contact"
      className="relative flex flex-col items-center justify-center overflow-hidden py-20 md:py-28"
    >
      <div className="relative z-10 mx-auto w-full max-w-full sm:max-w-md md:max-w-4xl px-4 sm:px-6">
        ...
        >
          ...
          <div className="absolute inset-0 rounded-2xl opacity-0 hover:opacity-100 transition-opacity duration-300 pointer-events-none blur-2xl bg-primary/20" />
